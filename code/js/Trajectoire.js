...
function ptsTrajectoire(MaScene,origineBalle,centreArceau1,centreArceau2,centreArceau3,positionCone, nbPoints,rayonCyl,hauteurCyl,hauteurCylHorizontal,bolDebutant, bolAffichePt){
    ...
    //erreur aléatoire si mode choisi = débutant

    let variationDifficulte = 0;
    let variationMaxDifficulte = 0;

    if(bolDebutant){
        let espaceArceaux = 1.7 * (hauteurCyl * 1.3 + rayonCyl * 1.3) + hauteurCylHorizontal - 2 * rayonCyl // Correspond à l'espace entre les deux cylindres d'un arceau (voir Arceau.js)
    ...
    // Points de contrôle pour la troisièm courbe de Bezier de degré 4 (arceau2 -> arceau3)
